...
%syntax: funcdef -> 'def' ident type_params parameters '->' test ':' [longstring] func_body_suite;
%syntax: funcdef -> 'def' ident type_params parameters ':' [longstring] func_body_suite;
...
%syntax: type_params -> '[' type_param_list ']';
%syntax: type_param_list -> type_param [',' type_param_list];
%syntax: type_param -> ident [':' test];
%syntax: type_param -> '*' ident;
%syntax: type_param -> '**' ident;
...
%syntax: small_stmt  -> expr_stmt | del_stmt | pass_stmt | flow_stmt | global_stmt | nonlocal_stmt | assert_stmt | type_stmt
...
%syntax: type_stmt -> 'type' ident [type_params] '=' test;
...
# Python 3.10: Match statement is now part of base grammar
%syntax: compound_stmt   -> if_stmt | while_stmt | for_stmt | try_stmt | with_stmt | funcdef | classdef | decorated | async_stmt | match_stmt;
...
# Simplified to avoid conflicts - supports single expression as subject
%syntax: match_stmt      -> 'match' test ':' EOL INDENT {case_block} DEDENT;
%syntax: case_block      -> 'case' patterns [guard] ':' suite;
%syntax: guard           -> 'if' test;

# Pattern syntax
%syntax: patterns        -> open_sequence_pattern;
%syntax: patterns        -> pattern;

%syntax: pattern         -> as_pattern;
%syntax: pattern         -> or_pattern;

%syntax: as_pattern      -> or_pattern 'as' ident;

%syntax: or_pattern      -> ['|'] closed_pattern ['|' or_pattern];

%syntax: closed_pattern  -> literal_pattern;
%syntax: closed_pattern  -> capture_pattern;
%syntax: closed_pattern  -> wildcard_pattern;
%syntax: closed_pattern  -> value_pattern;
%syntax: closed_pattern  -> group_pattern;
%syntax: closed_pattern  -> sequence_pattern;
%syntax: closed_pattern  -> mapping_pattern;
%syntax: closed_pattern  -> class_pattern;

%syntax: literal_pattern -> signed_number;
%syntax: literal_pattern -> stringliteral;
%syntax: literal_pattern -> 'None';
%syntax: literal_pattern -> 'True';
%syntax: literal_pattern -> 'False';

%syntax: signed_number   -> ['-'] NUMBER;

%syntax: capture_pattern -> ident;

%syntax: wildcard_pattern -> '_';

# value_pattern requires at least one dot to distinguish from capture_pattern
%syntax: value_pattern   -> dotted_ident '.' ident;

%syntax: group_pattern   -> '(' pattern ')';

%syntax: sequence_pattern -> '[' [maybe_sequence_pattern] ']';
%syntax: sequence_pattern -> '(' [open_sequence_pattern] ')';

%syntax: open_sequence_pattern -> maybe_star_pattern ',' [maybe_sequence_pattern];

%syntax: maybe_sequence_pattern -> [maybe_sequence_pattern ','] maybe_star_pattern;

%syntax: maybe_star_pattern -> star_pattern;
%syntax: maybe_star_pattern -> pattern;

%syntax: star_pattern    -> '*' ident;
%syntax: star_pattern    -> '*' '_';

%syntax: mapping_pattern -> '{' [items_pattern] '}';
%syntax: items_pattern   -> [items_pattern ','] key_value_pattern;
%syntax: items_pattern   -> [items_pattern ','] double_star_pattern;

%syntax: key_value_pattern -> (literal_pattern | value_pattern) ':' pattern;
%syntax: double_star_pattern -> '**' ident;

%syntax: class_pattern   -> dotted_ident '(' [pattern_arguments] ')';
%syntax: pattern_arguments -> positional_patterns [',' keyword_patterns];
%syntax: pattern_arguments -> keyword_patterns;

%syntax: positional_patterns -> [positional_patterns ','] pattern;
%syntax: keyword_patterns -> [keyword_patterns ','] keyword_pattern;
%syntax: keyword_pattern -> ident '=' pattern;
...
%syntax: classdef -> 'class' ident type_params ['(' [arglist] ')'] ':' suite;
